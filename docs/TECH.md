...
GitHub Actions cron (UTC 00:17)
  ...
  │
  ├─ 3. analyzer.analyze_repo()          逐个项目调 LLM 做 7 维结构化分析
...
- `analyze_repo(repo)` — 单项目分析，输出 7 维 JSON：
...
- **超时**：100 分钟
- **Secrets**：`LLM_API_KEY`（必填）、`LLM_BASE_URL`、`LLM_MODEL`、`DAILY_REPO_LIMIT`、`GITHUB_TOKEN`
